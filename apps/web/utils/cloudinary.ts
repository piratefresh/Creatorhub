...
export async function uploadFile(file: File) {
  ...
  formData.append("resource_type", getResourceType(file)); // set resource type dynamically

  const response = await fetch(
    `https://api.cloudinary.com/v1_1/${signData.cloudName}/${getResourceType(
      file
    )}/upload`, // use dynamic file type in the URL
    {
      method: "POST",
      body: formData,
    }
